var appServices = angular.module('gw2assistant.services', [])
...
.service('GuidesList', function ($rootScope, GuidesLoader, $q){
	this.get = function(guidesSourceUrl) {
		var response = $q.defer();
		//num is the number of results to pull form the source
		GuidesLoader.fetch({q: guidesSourceUrl, num: 20}, {}, function (data){
			response.resolve(data.responseData);
		});
		return response.promise;
	};
...
.service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService, API_KEY){
  ...
  this.getUserGravatar = function(userId){
    ...
    $http.jsonp(WORDPRESS_API_URL + 'userplus/get_avatar/' +
    '?key=56564b3e150c6' +
    '&user_id='+ userId +
    '&type=full' +
    '&callback=JSON_CALLBACK')
  ...
  this.submitComment = function(postId, content) {
    ...
    $http.jsonp(WORDPRESS_API_URL + 'userplus/post_comment/' +
    '?key='+ $API_KEY +
    '&post_id='+ postId +
    '&cookie='+ user.cookie +
    '&comment_status=1' +
...
// bbPress RELATED FUNCTIONS
/*
.service('ForumsService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService, API_KEY){

  this.getForums = function(forumId, page) {
    var deferred = $q.defer();

    $http.jsonp(WORDPRESS_API_URL + 'bbpress/get_forums/' +
    '?id='+ forumId +
    '&page='+ page +
    '&callback=JSON_CALLBACK')
    .success(function(data) {
...
.service('BookMarkService', function (_, $rootScope){
	...
	this.bookmarkFeedPost = function(bookmark_post){
		...
		this.bookmarkGuidesPost = function(bookmark_post){

		var user_bookmarks = !_.isUndefined(window.localStorage.ionFullApp_guides_bookmarks) ?
														JSON.parse(window.localStorage.ionFullApp_guides_bookmarks) : [];
		...
		window.localStorage.ionFullApp_guides_bookmarks = JSON.stringify(user_bookmarks);
		$rootScope.$broadcast("new-bookmark");
	};
	...
	this.getBookmarks = function(){
		return {
			...
			guides : JSON.parse(window.localStorage.ionFullApp_guides_bookmarks || '[]'),
...
.service('AuthService', function ($rootScope, $http, $q, WORDPRESS_API_URL){
  ...
  this.validateAuth = function(user) {
    var deferred = $q.defer();
    $http.jsonp(WORDPRESS_API_URL + 'userplus/validate_auth_cookie/' +
    '?key=56564b3e150c6' +
    '&cookie='+ user.cookie +
    '&callback=JSON_CALLBACK')
    .success(function(data) {
  ...
  this.doLogin = function(user) {
    ...
    authService.requestNonce("userplus", "generate_auth_cookie")
    .then(function(nonce){
  ...
  this.doRegister = function(user) {
    ...
    authService.requestNonce("userplus", "register")
    .then(function(nonce){
      nonce_dfd.resolve(nonce);
    });
    ...
    nonce_dfd.promise.then(function(nonce){
      authService.registerUser(user.userName, user.email,
        user.displayName, user.passworwd, nonce)
      .then(function(data){
        if(data.status == "error"){
          // return error message
          deferred.reject(data.error);
  ...
  this.doForgotPassword = function(username) {
    var deferred = $q.defer();
    $http.jsonp(WORDPRESS_API_URL + 'userplus/retrieve_password/' +
    '?key=56564b3e150c6' +
    '&user_login='+ username +
    '&callback=JSON_CALLBACK')
    .success(function(data) {
  ...
  this.generateAuthCookie = function(username, password, nonce) {
    var deferred = $q.defer();
    $http.jsonp(WORDPRESS_API_URL + 'userplus/generate_auth_cookie/' +
    '?key=56564b3e150c6' +
    '&username='+ username +
    '&password=' + password +
    '&nonce='+ nonce +
  ...
  this.getUser = function(){
    ...
    return {
      ...
      name : JSON.parse(window.localStorage.ionWordpress_user_avatar || null),
      data: data,
      cookie: cookie
    };
  };
  ...
  this.registerUser = function(username, email, displayName, password, nonce) {
    var deferred = $q.defer();
    $http.jsonp(WORDPRESS_API_URL + 'userplus/register/' +
    '?key=56564b3e150c6' +
    '&username='+ username +
    '&email=' + email +
